...
const {listingDTO, fetchListingUpdateData, initialData, fetchInitialData, setPhoneRaw, updateListing, rooms, createListing, validationFail, createOrUpdateError} = useCreateListing()
		...
		<div class="create mt-8">
			<v-form ref="form">
				...
				<v-card class="set-price-listingDTO mt-4" v-if="!isGuestHouse">
					<v-card-item>
						<v-text-field class="mt-4" v-model.number="listingDTO.minPrice" label="Минимальная цена"/>
						<PeriodsCreater v-model="listingDTO.pricePeriods"/>
					</v-card-item>
				</v-card>
				<CreateRooms v-else edit/>
