<script setup lang="ts">
	...
	const {setAddress, listingDTO} = useCreateListing()
	...
	if (listingDTO.value.coords.longitude && listingDTO.value.coords.width && listingDTO.value.address !== '') {
		selectedAddress.value = {
			properties: {description: "", name: listingDTO.value.address},
			geometry: {
				coordinates: [listingDTO.value.coords.longitude, listingDTO.value.coords.width],
				type: 'Point'
			}
		}
	}
...
<template>
	<v-card class="address mt-4" title="Ввод адреса">
		...
		<v-card-item>
			...
			<v-window v-model="tab">
				<v-window-item :value="1">
					<v-text-field class="mt-4" v-model="searchQuery" label="Полный адрес"/>
				...
				<v-window-item :value="2">
					<v-text-field class="mt-4" v-model="addressInput" label="Полный адрес"/>
					<v-text-field v-model="cordsInput" label="Координаты из яндекса"/>
					<v-btn color="blue" class="mb-4" @click="setCoords">Сохранить</v-btn>
				</v-window-item>
				<div class="map listingDTO-block" v-if="selectedAddress">
